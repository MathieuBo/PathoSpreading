...
def stability(exp_data, connectivity_ipsi, connectivity_contra, nb_region, timepoints, c_rng, individual=False):
    """
...
def stability_individual(exp_data, connectivity_ipsi, connectivity_contra, nb_region, timepoints, c_rng):
    ...
    for regions in tqdm(range(0, nb_region + 1)):
        ...
        ROInames = ipsi_sec + contra_sec
        if regions > 3:
            L = Laplacian.get_laplacian(adj_mat=W, expression_data=None,
                                        return_in_degree=False)  # Need min 2 values for a valid Laplacian
            # print("Len L", len(L))
            ind_list = []
            ###############
            for nb_mice in range(0,len(ind_grp.columns)):
                ind_grp = process_files.mean_pathology(timepoints=timepoints, path_data=path_data, individual=True)
                c, r = fitfunctions.c_fit(log_path=np.log10(grp_mean), L_out=L, tp=timepoints, seed='iCPu', c_rng=c_rng,
                                      roi_names=ROInames)
            # List of the c and r values
                c_stab.append(c)
